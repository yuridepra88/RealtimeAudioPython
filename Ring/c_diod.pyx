...
mod_cdata = <double *>malloc(2048*cython.sizeof(double))

@cython.boundscheck(False)
@cython.wraparound(False)
def compute_c1(np.ndarray[double, ndim=1, mode="c"] data not None, np.ndarray[double, ndim=1, mode="c"] mod_data not None, int FRAMES_PER_BUFFER, float T, int over_f, np.ndarray[double, ndim=1, mode="c"] a, np.ndarray[double, ndim=1, mode="c"] b, np.ndarray[double, ndim=1, mode="c"] zi):
